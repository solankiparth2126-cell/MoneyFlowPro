                ...
                <CardContent className="flex-1 flex flex-col justify-center py-2">
                    <div className="grid grid-cols-7 gap-1.5 mx-auto">
                        {Array.from({ length: 28 }).map((_, i) => (
                            <div key={i} className="h-6 w-6 rounded-md bg-gray-100 dark:bg-gray-800 animate-pulse" />
                        ))}
                    </div>
                </CardContent>
            </Card>
                        ...
                        <TooltipProvider key={day.date}>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <div
                                        className={`h-6 w-6 rounded-md ${intensityColors[day.intensity]} transition-all hover:scale-110 cursor-pointer flex items-center justify-center text-[7px] font-bold`}
                                    >
